...
from file_logistics import ndata_chaos, plotter_setup
...
class gw_calcs(object):
    ...
    def scatter_hist(self, x1, y1, x2, y2, ax, ax_histf, ax_histh, 
                     label1, label2, data_exist, data_filt):
        """
        ...
        ax_histf.plot(kdef_SMBH.support, kdef_SMBH.density, 
                      color='blueviolet', label=label1)
        ax_histf.fill_between(kdef_SMBH.support, kdef_SMBH.density, 
                              alpha=0.35, color='blueviolet')
        ...
        ax_histh.fill_between(kdeh_SMBH.density, kdeh_SMBH.support, 
                              alpha=0.35, color='blueviolet')
        ...
        if (data_exist):
            ...
            ax_histf.plot(kdef_IMBH.support, kdef_IMBH.density, 
                          color='orange', label=label2)
            ax_histf.fill_between(kdef_IMBH.support, kdef_IMBH.density, 
            ...
            ax_histh.fill_between(kdeh_IMBH.density, kdeh_IMBH.support, 
                                  alpha=0.35, color='orange')
    ...
    def GW_event_tracker(self):
        """
        Plot all GW events into a histogram.
    ...
    def orbital_hist_plotter(self):
        ...
        for fold_ in self.folders[:self.frange]:
                ...
                for parti_ in range(len(self.semi_flyby_nn)): #Looping through every individual particle
                    for event_ in range(len(self.semi_flyby_nn[parti_])): #Looping through every detected event
                        ...
                        if np.asarray(self.fb_nn_SMBH[parti_][event_])<0 \
                            and self.mass_IMBH[parti_]<1e5 | units.MSun:
            ...
            const_tgw2 = [(rmass_SMBH/rmass_IMBH)*i for i in const_tgw]
            ...
            ax.text(-4.0, -2.9, r'$\mu$Ares ($f_{\rm{peak}} = 10^{-3}$ Hz)', 
                    verticalalignment='center', fontsize=axlabel_size, 
                    rotation=self.text_angle+17, color='white')
            ax.text(-5.15, -3.4, r'LISA ($f_{\rm{peak}} = 10^{-2}$ Hz)', 
                    verticalalignment='center', fontsize=axlabel_size, 
                    rotation=self.text_angle+17, color='white')
            for int_ in range(drange):
                ax.scatter(np.log10(SMBH_sem[int_]), SMBH_ecc[int_], 
                           color=self.colors[int_+iterf], 
                           s=0.75, zorder=5)
                ...
                ax2.fill_between(kdeh_SMBH.density, kdeh_SMBH.support, 
                                 alpha=0.35, color=self.colors[int_+iterf])
            ...
            ax.text(-1.5, -2.3, r'$t_{\rm{GW}}>t_H$', verticalalignment='center', 
                    fontsize=axlabel_size+4, rotation=self.text_angle+15, color='white', 
                    path_effects=[pe.withStroke(linewidth=2, foreground="black")], zorder=6)
            ax.text(-1.7, -3.3, r'$t_{\rm{GW}}<t_H$', verticalalignment='center', 
                    fontsize=axlabel_size+4, rotation=self.text_angle+15, color='white', 
                    path_effects=[pe.withStroke(linewidth=2, foreground="black")], zorder=6)
            ...
            ax.text(-5.15, -2.9, r'$\mu$Ares ($f_{\rm{peak}} = 10^{-3}$ Hz)', 
                    verticalalignment='center', fontsize=axlabel_size, 
                    rotation=self.text_angle+17, color='white', zorder=8)
            ax.text(-5.15, -4.6, r'LISA ($f_{\rm{peak}} = 10^{-2}$ Hz)', 
                    verticalalignment='center', fontsize=axlabel_size, 
                    rotation=self.text_angle+17, color='white')
            self.forecast_interferometer(ax, IMBH_mass, IMBH_mass)
            for int_ in range(drange):
                ax.scatter(np.log10(IMBH_sem[int_]), IMBH_ecc[int_], 
                           color=self.colors[int_+iterf], s=0.75, zorder=5)
                ...
                ax2.fill_between(kdeh_IMBH.density, kdeh_IMBH.support, 
                                 alpha=0.35, color=self.colors[int_+iterf])
                
                kdef_IMBH = sm.nonparametric.KDEUnivariate(np.log10(IMBH_sem[0][:no_data_IMBH]))
                kdef_IMBH.fit()
                kdef_IMBH.density = (kdef_IMBH.density/max(kdef_IMBH.density))
                ax1.plot(kdef_IMBH.support, kdef_IMBH.density, 
                         color=self.colors[int_+iterf], 
                         label=integrator[int_])
                ax1.fill_between(kdef_IMBH.support, kdef_IMBH.density, 
                                 alpha=0.35, color=self.colors[int_+iterf])
            ...
            ax.text(-1.6, -4.1, r'$t_{\rm{GW}}>t_H$', verticalalignment='center', 
                    fontsize=axlabel_size+4, rotation=self.text_angle+14, color='white', 
                    path_effects=[pe.withStroke(linewidth=2, foreground="black")], zorder=6)
            ax.text(-1.6, -5.4, r'$t_{\rm{GW}}<t_H$', verticalalignment='center', 
                    fontsize=axlabel_size+4, rotation=self.text_angle+14, color='white', 
                    path_effects=[pe.withStroke(linewidth=2, foreground="black")], zorder=6)
